...
interface ProviderListProps {
  ...
  /** 删除当前明确选中状态下的供应商；确认框由列表统一处理 */
  onBulkDelete?: (providers: Provider[]) => void | Promise<void>;
...
export const ProviderList = forwardRef<ProviderListHandle, ProviderListProps>(
  function ProviderList(
    ...
    const [cleanupCandidates, setCleanupCandidates] = useState<
      Provider[] | null
    >(null);
    const [isCleaningByStatus, setIsCleaningByStatus] = useState(false);
    ...
    const filteredProviders = useMemo(() => {
      ...
      return displayProviders.filter((provider) => {
        ...
        const effectiveProbeFilter = onlyAvailable
          ? "success"
          : probeStatusFilter;
        ...
        if (
          effectiveProbeFilter === "failed" &&
          failureReasonFilter !== "all" &&
          (modelsProbeHistoryById[provider.id]?.reason || "unknown") !==
            failureReasonFilter
        ) {
          return false;
        }
    ...
    /** Fixed order so cleanup filters stay discoverable (esp. API Key errors). */
    ...
    const failureReasonOptions = useMemo(() => {
      ...
      for (const entry of Object.values(modelsProbeHistoryById)) {
        if (entry.status !== "failed") continue;
        const reason = entry.reason || "unknown";
        counts.set(reason, (counts.get(reason) ?? 0) + 1);
      }
      // Always include known categories (count may be 0) so "API Key 错误" is
      // available for one-click cleanup even before any such record exists.
      for (const reason of KNOWN_FAILURE_REASONS) {
        if (!counts.has(reason)) counts.set(reason, 0);
      }
      return Array.from(counts.entries())
        .map(([reason, count]) => ({ reason, count }))
        .sort((a, b) => {
          const ai = (KNOWN_FAILURE_REASONS as readonly string[]).indexOf(
            a.reason,
          );
          const bi = (KNOWN_FAILURE_REASONS as readonly string[]).indexOf(
            b.reason,
          );
    ...
    const statusCleanupProviders = useMemo(() => {
      // success providers are valid; never offer bulk delete for that status
      if (probeStatusFilter === "all" || probeStatusFilter === "success") {
        return [];
      }
      return displayProviders.filter((provider) => {
        const entry = modelsProbeHistoryById[provider.id];
        const matchesStatus =
          probeStatusFilter === "unchecked"
            ? !entry?.status
            : entry?.status === probeStatusFilter;
        if (!matchesStatus) return false;
        return !(
          probeStatusFilter === "failed" &&
          failureReasonFilter !== "all" &&
          (entry?.reason || "unknown") !== failureReasonFilter
        );
      });
    }, [
      displayProviders,
    ...
    const handleConfirmStatusCleanup = useCallback(async () => {
      const candidates = cleanupCandidates ?? [];
      if (!onBulkDelete || candidates.length === 0) {
        setCleanupCandidates(null);
        return;
      }
      setIsCleaningByStatus(true);
      try {
        await onBulkDelete(candidates);
        setCleanupCandidates(null);
      } finally {
            ...
            {filteredProviders.map((provider) => {
              ...
              return (
                <SortableProviderCard
                  ...
                  modelsProbeReason={
                    modelsProbeById[provider.id]?.reason ??
                    modelsProbeHistoryById[provider.id]?.reason
                  }
                  modelsProbeModelIds={
    ...
    return (
      <div ref={listRootRef} className="space-y-4">
            ...
            >
              ...
              <PopoverContent className="w-72 space-y-3 p-3" align="end">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(
                      ...
                      <Button
                        ...
                        size="sm"
                        variant={
                          probeStatusFilter === value ? "default" : "outline"
                        }
                        className="h-7 px-2 text-xs"
                        onClick={() => {
                          setProbeStatusFilter(value);
                          setOnlyAvailable(value === "success");
                          if (value !== "failed") setFailureReasonFilter("all");
                        }}
                      >
                ...
                {probeStatusFilter === "failed" && (
                  <div className="space-y-1.5">
                    <div className="text-xs font-medium text-muted-foreground">
                      {t("provider.filterByFailureReason", {
                        defaultValue: "失败原因",
                      })}
                    </div>
                    <div className="flex flex-wrap gap-1">
                      <Button
                        ...
                        size="sm"
                        variant={
                          failureReasonFilter === "all" ? "default" : "outline"
                        }
                        className="h-7 px-2 text-xs"
                        onClick={() => setFailureReasonFilter("all")}
                      >
                        {t("provider.failureReason.all", {
                          defaultValue: "全部原因",
                        })}
                      </Button>
                      {failureReasonOptions.map(({ reason, count }) => (
                        <Button
                          key={reason}
                          type="button"
                          size="sm"
                          variant={
                            failureReasonFilter === reason
                              ? "default"
                              : "outline"
                          }
                          className={
                            count === 0
                              ? "h-7 px-2 text-xs opacity-60"
                              : "h-7 px-2 text-xs"
                          }
                          onClick={() => setFailureReasonFilter(reason)}
                        >
                          {t(`provider.failureReason.${reason}`, {
                            defaultValue:
                              reason === "api_key" ? "API Key 错误" : reason,
                          })}{" "}
                          {count}
                        </Button>
                      ))}
                ...
                {onBulkDelete &&
                  probeStatusFilter !== "all" &&
                  probeStatusFilter !== "success" && (
                    <Button
                      type="button"
                      variant="destructive"
                      size="sm"
                      className="h-8 w-full gap-1.5 text-xs"
                      disabled={statusCleanupProviders.length === 0}
                      onClick={() => {
                        // Close filter popover first so ConfirmDialog is clearly a second step.
                        setFilterPopoverOpen(false);
                        setCleanupCandidates(statusCleanupProviders);
                      }}
                    >
                      <Trash2 className="h-3.5 w-3.5" />
                      {t("provider.cleanupByStatus", {
                        count: statusCleanupProviders.length,
                        defaultValue: `一键清理此状态（${statusCleanupProviders.length}）`,
                      })}
                    </Button>
                  )}
              </PopoverContent>
        ...
        <ConfirmDialog
          isOpen={Boolean(cleanupCandidates)}
          title={t("provider.cleanupByStatusConfirmTitle", {
            defaultValue: "按状态清理供应商",
          })}
          message={t(
            probeStatusFilter === "failed" && failureReasonFilter !== "all"
              ? "provider.cleanupByStatusConfirmMessageWithReason"
              : "provider.cleanupByStatusConfirmMessage",
            {
              count: cleanupCandidates?.length ?? 0,
              status: t(`provider.filterStatus.${probeStatusFilter}`, {
                defaultValue: probeStatusFilter,
              }),
              reason:
                probeStatusFilter === "failed" && failureReasonFilter !== "all"
                  ? t(`provider.failureReason.${failureReasonFilter}`, {
                      defaultValue: failureReasonFilter,
                    })
                  : "",
              defaultValue:
                probeStatusFilter === "failed" && failureReasonFilter !== "all"
                  ? `将永久删除 ${cleanupCandidates?.length ?? 0} 个“${probeStatusFilter} / ${failureReasonFilter}”状态的供应商，此操作无法撤销。`
                  : `将永久删除 ${cleanupCandidates?.length ?? 0} 个“${probeStatusFilter}”状态的供应商，此操作无法撤销。`,
            },
          )}
          checkboxLabel={t("provider.cleanupByStatusConfirmCheck", {
            defaultValue: "我已确认这些供应商可删除，且操作无法撤销",
          })}
          checkboxDefaultChecked={false}
          requireCheckbox
          confirmText={
            isCleaningByStatus
              ? t("provider.cleaningByStatus", { defaultValue: "清理中…" })
              : t("provider.confirmCleanupByStatus", {
                  defaultValue: "确认清理",
                })
          }
          onConfirm={(checked) => {
            if (!checked) {
              // Second step is incomplete until the checkbox is ticked.
              return;
            }
            void handleConfirmStatusCleanup();
          }}
          onCancel={() => {
