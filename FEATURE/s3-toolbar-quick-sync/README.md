...
> 文档状态：最后同步 2026-07-15 · 供应商页工具栏一键 S3 上传/下载

## 目标

在供应商页顶部工具栏，**「一键拉取模型」左侧**增加两个 S3 兼容存储快捷按钮：
...
```text
[S3上传] [S3下载] [一键拉取模型] [定位当前] [AppSwitcher] ...
```
...
若已启用用量日聚合同步，见 [sync-usage-daily-rollups](../sync-usage-daily-rollups/)。
