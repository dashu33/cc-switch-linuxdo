...
| 文件                                        | 角色                                                                                            |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `src/App.tsx`                               | 准星按钮、`providerListRef`、`handleScrollToCurrentProvider` 多拍重试；按钮通过操作插槽传入列表 |
| `src/components/providers/ProviderList.tsx` | sticky 子菜单操作区、`ProviderListHandle.scrollToCurrentProvider`、搜索清空、祖先滚动           |
...
```text
Provider submenu Crosshair button (App.tsx → ProviderList toolbarActions)
