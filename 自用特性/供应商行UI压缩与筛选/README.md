...
- 选择“失败”后可继续按稳定失败原因筛选；原因芯片固定列出（含「API Key 错误」），数量为 0 时仍可选，便于定向清理。
- 选择失败 / 无模型 / 跳过 / 未检测后显示“一键清理此状态”；**成功状态不提供清理**。失败状态可叠加原因。清理前展示数量并二次确认（关闭筛选弹层后弹出确认框，需勾选「确认可删除」后才能执行）；“全部”也不提供清理。
- 行内「获取」失败时，按钮右侧显示稳定失败原因（持久化）；细节见 [../批量拉取模型探测/](../批量拉取模型探测/)。

## 关键决策

- probe history 键仍为 `cc-switch-models-probe-history:v1:<appId>`，只**扩展**字段，不改 key，保证旧数据兼容。
- 失败历史只持久化稳定分类码，不保存后端原始错误字符串；旧失败记录没有 `reason` 时归入“其他或旧记录”。
- 按状态清理逐条复用既有删除 API，但只做一次列表/关联缓存/托盘刷新；已删除项同时从探测历史移除。
...
- [ ] 失败状态可按原因二次筛选，旧失败记录落入“其他或旧记录”
- [ ] 失败/无模型/跳过/未检测可一键清理，数量正确、二次确认；成功与“全部”状态无清理按钮
...
- [ ] `pnpm typecheck` 通过
- [ ] 相关单测通过
- [ ] Claude / GPT / Gemini / Grok 的真实版本、日期后缀和轻量变体能选中最新旗舰模型
...
- 点击品牌 LOGO 不再固定取探测列表“第一个”，而是优先旗舰/通用型号，再按语义模型版本、档位、稳定性和发布日期选择顶级模型。
- 日期（如 `20251001`）、上下文长度和参数量不再误作模型版本；`mini` / `flash` / `haiku` / 搜索 / 图像 / 音频等轻量或专用型号会降级。
- `preview` / `beta` 不做硬降级：新一代旗舰预览版可胜过旧一代稳定版，同版本同档位时仍优先稳定版。
...
- Claude / Claude Desktop 非官方卡同样显示：上游格式下拉 + 模型下拉 + 获取 + 品牌 LOGO
- 与 Codex 共用 `CodexProviderQuickAdjust`（按 `appId` 切换读写字段与格式选项）
- 信息列不再为上述应用重复渲染 LOGO 网格
